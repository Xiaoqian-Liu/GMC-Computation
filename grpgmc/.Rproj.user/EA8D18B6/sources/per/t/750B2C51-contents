#'Cross-validation for grouped variable selection with GMCpenalization
#'
#' \code{cv.grGMC} performs k-fold cross-validation for penalizaed linear regression models with the groupe GMC penalty
...
#' fit_GMC <- cv.grGMC(y, A, theta=0.9, group, lambda, K)

cv.grGMC <- function(y, A, theta, group=1:ncol(A), lambda, group.multiplier, Z.method = "theta",
                     Z.epsilon = 1e-3,  nfolds=10, seed=1234, maxiters = 1e4, tol = 1e-5,
  ...
  cv.args <- list()
  cv.args$theta <- theta
  cv.args$Z.method <- Z.method
  cv.args$Z.epsilon <- Z.epsilon
  ...
  for (i in 1:nfolds) {
    ...
    res <- cv_fold(i, newy, newA, fold, cv.args)
...
cv_fold <- function(i, y, A, fold, cv.args) {
  cv.args$A <- A[fold!=i, , drop=FALSE]
  cv.args$y <- y[fold!=i]
  fit.i <- do.call("grGMC", cv.args)
